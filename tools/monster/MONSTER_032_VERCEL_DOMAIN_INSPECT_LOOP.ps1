...
Write-Host "=== VERCEL DOMAINS INSPECT LOOP ===" -ForegroundColor Cyan
...
while ((Get-Date) -lt $end) {
  ...
  $ok1 = ($text.ToLowerInvariant() -match [regex]::Escape($ExpectNS1.ToLowerInvariant()))
  $ok2 = ($text.ToLowerInvariant() -match [regex]::Escape($ExpectNS2.ToLowerInvariant()))
  $bad = ($text -match "☓")

  if ($ok1 -and $ok2 -and (-not $bad)) { Ok "Vercel shows intended==current nameservers (no ☓)."; break }
  Warn "Vercel still reports mismatch (or ☓). Waiting..."
  Start-Sleep -Seconds $Interval
}
Ok "Inspect loop finished."
