...
$target = Join-Path (Get-Location) "D8_IO_MEGA_DOCTOR_ALLINONE_013_FIX.ps1"

# NOTE: no here-string nesting; we write the script as plain text lines.
$scriptLines = @(
'Set-StrictMode -Version Latest',
'$ErrorActionPreference = "Stop"',
'',
...
'"}"' ,
') -join "`n"',
'$pingContent = "export const runtime = ""nodejs"";`nexport const dynamic = ""force-dynamic"";`nexport async function GET(){ return new Response(""PONG: ' + '" + $Stamp + "' + '\n"", {status:200, headers:{""content-type"":""text/plain; charset=utf-8"",""x-d8-proof"":""' + '" + $Stamp + "' + '""}}); }"',
'$stateContent = "export const runtime = ""nodejs"";`nexport const dynamic = ""force-dynamic"";`nexport async function GET(){ return new Response(JSON.stringify({ok:true,stamp:""' + '" + $Stamp + "' + '""}), {status:200, headers:{""content-type"":""application/json; charset=utf-8"",""x-d8-proof"":""' + '" + $Stamp + "' + '""}}); }"',
...
'"    if ($code -eq 200 -and $proof) { Write-Host (""GREEN {0} proof={1}"" -f (Get-Date -Format ""HH:mm:ss""), $proof) -ForegroundColor Green } else { Write-Host (""RED   {0} http={1} proof={2}"" -f (Get-Date -Format ""HH:mm:ss""), $code, $proof) -ForegroundColor Red }",',
...
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText($target, ($scriptLines -join "`r`n"), $utf8NoBom)

Ok ("WROTE: " + $target)
Get-Item -LiteralPath $target | Select-Object FullName,Length,LastWriteTime | Format-List