...
## **6. This project's compliance (Dominat8.io)**

| Check | Status |
|-------|--------|
| `clerkMiddleware()` in proxy | ✅ Root `proxy.ts` uses `clerkMiddleware()` from `@clerk/nextjs/server` (with custom domain/admin/rewrite logic). |
| `<ClerkProvider>` in layout | ✅ `src/app/layout.tsx` wraps the app with `<ClerkProvider>`. |
| Clerk components in layout/pages | ✅ `<SignedIn>`, `<SignedOut>`, `<SignInButton>`, `<SignUpButton>`, `<UserButton>` used in `AuthHeader` (client component) rendered from the layout. |
| Imports from `@clerk/nextjs` or `@clerk/nextjs/server` | ✅ All Clerk imports use these packages. |
| App Router only | ✅ No `_app.tsx`; routes under `app/`. |
| `auth()` with async/await | ✅ API routes and server layouts use `await auth()` from `@clerk/nextjs/server` (with try/catch for Next 16 compatibility). |
| Keys in `.env.local` only, placeholders in docs | ✅ `.env.example` uses placeholders; real keys only in `.env.local` / Vercel env. |

**Intentional variations (still compliant):**

- **Proxy:** Custom logic (domain rewrites, admin-key protection, `/io` rewrite) is inside `clerkMiddleware(async (auth, request) => { ... })`; the proxy still uses `clerkMiddleware()`.
- **Auth UI:** Sign-in/sign-up/UserButton live in a **client component** (`AuthHeader`) to avoid Next.js 16 server-side `auth()`/headers issues; they are used from the root layout.
- **Optional Clerk:** If `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` is unset, the proxy uses a pass-through handler so the app does not crash (see root `proxy.ts`).
