// app/api/projects/[projectId]/debug/spec/route.ts
...
async function getKvClient(): Promise<any> {
  // Most likely in a Vercel KV setup
  try {
    const mod = await import("@vercel/kv");
    if ((mod as any).kv) return (mod as any).kv;
  } catch {}

  // Try common local helper paths (best-effort; won't crash the server if missing)
  const candidates = [
    "../../../../../../src/app/lib/kv",
    "../../../../../../app/lib/kv",
    "../../../../../../src/lib/kv",
  ...
  throw new Error(
    "KV client not found. Ensure @vercel/kv is installed and env vars are set, or export kv from a local helper."
  );
}
...
export async function GET(
  ...
  try {
    ...
    // Try a short list of likely keys (reports hits; doesn't assume correctness)
    const candidates = [
