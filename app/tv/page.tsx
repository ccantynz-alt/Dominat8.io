...
export default function Tv() {
  ...
  const healthUrl = useMemo(
    () => (baseMissing ? "" : `${baseRaw}/healthz`),
    [baseRaw, baseMissing]
  );
  const stampUrl = useMemo(
    () => (baseMissing ? "" : `${baseRaw}/api/debug/stamp`),
    [baseRaw, baseMissing]
  );
  ...
  const promoteCmd = `powershell -NoProfile -ExecutionPolicy Bypass \\
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: 6, opacity: 0.85 }}>
          at: {health?.at || "-"} · status: {health?.status ?? "-"}
        </div>
        <pre
          style={{
            marginTop: 10,
      ...
      >
        ...
        <div style={{ marginTop: 6, opacity: 0.85 }}>
          at: {stamp?.at || "-"} · status: {stamp?.status ?? "-"}
        </div>
        <pre
          style={{
            marginTop: 10,
