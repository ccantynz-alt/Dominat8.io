...
const PLANS = [
  ...
  },
  {
    ...
    accent: "rgba(232,180,79,0.05)",
    accentBorder: "rgba(232,180,79,0.22)",
    accentBtn: "rgba(232,180,79,0.18)",
    accentBtnBorder: "rgba(232,180,79,0.35)",
  ...
  },
  {
    ...
    accent: "rgba(124,90,255,0.06)",
    accentBorder: "rgba(124,90,255,0.28)",
    accentBtn: "linear-gradient(135deg,#7C5AFF,#6347FF)",
  ...
  },
  {
    ...
    accent: "rgba(80,70,228,0.06)",
    accentBorder: "rgba(80,70,228,0.28)",
    accentBtn: "rgba(80,70,228,0.22)",
    accentBtnBorder: "rgba(80,70,228,0.40)",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 14, maxWidth: 1160, margin: "0 auto", padding: "0 24px" }}>
        {PLANS.map((plan) => (
          <div key={plan.name} style={{
            ...
            padding: 22,
            position: "relative",
            boxShadow: plan.highlight ? "0 0 0 1px rgba(124,90,255,0.18), 0 24px 60px rgba(0,0,0,0.40)" : "0 8px 32px rgba(0,0,0,0.25)",
          }}>
            {plan.badge && (
              <div style={{ position: "absolute", top: -12, left: "50%", transform: "translateX(-50%)", padding: "4px 14px", borderRadius: 999, background: "linear-gradient(135deg,#7C5AFF,#6347FF)", color: "#fff", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", whiteSpace: "nowrap" }}>
                {plan.badge}
              </div>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              {plan.features.map((f) => (
                <div key={f} style={{ display: "flex", alignItems: "flex-start", gap: 7, fontSize: 12, color: "rgba(255,255,255,0.72)", lineHeight: 1.4 }}>
                  <span style={{ color: "rgba(52,211,153,0.90)", flexShrink: 0, marginTop: 1 }}>✓</span>
                  {f}
                </div>
              ))}
            </div>
        ...
        <div style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", background: "rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            {["Free", "Starter", "Pro", "Agency"].map((h) => (
              <div key={h} style={{ padding: "11px 12px", textAlign: "center", fontSize: 12, fontWeight: 700, color: h === "Pro" ? "rgba(124,90,255,0.90)" : h === "Starter" ? "rgba(232,180,79,0.85)" : h === "Agency" ? "rgba(80,70,228,0.85)" : "rgba(255,255,255,0.55)" }}>{h}</div>
            ))}
          </div>
          {COMPARISON.map((row, i) => (
            <div key={row.feature} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", borderBottom: i < COMPARISON.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.02)" }}>
              ...
              {[row.free, row.starter, row.pro, row.agency].map((val, j) => (
                <div key={j} style={{ padding: "10px 12px", textAlign: "center", fontSize: 12, color: val === "✓" ? "rgba(52,211,153,0.85)" : val === "—" ? "rgba(255,255,255,0.18)" : "rgba(255,255,255,0.78)" }}>{val}</div>
              ))}
            </div>
          ))}
        </div>
