...
function publishedKey(projectId: string) {
  ...
  return `published:project:${projectId}:latest`;
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // âœ… default dryRun to FALSE (real publish) unless explicitly dryRun:true
  ...
  // AUDIT â€” OPTIONAL, SKIPPED IF MISSING
  ...
  // REAL PUBLISH
  // âœ… IMPORTANT: prevent infinite recursion:
  // publish preset calls auto-publish, so auto-publish MUST call publish with autoPublish:false.
  let publishStep = await callJsonStep({
    name: "publish",
    url: routes.publish,
    method: "POST",
    body: { preset: "publish", autoPublish: false },
  });
  ...
  if (!publishStep.ok && missingSpec) {
    ...
    if (!seed.ok) {
      return res.status(500).json({ ok: false, projectId, agent: "auto-publish", buildTag, gitSha, steps, writtenKeys });
    }

    const retry = await callJsonStep({
      name: "publish-retry",
      url: routes.publish,
      method: "POST",
      body: { preset: "publish", autoPublish: false },
  ...
  // âœ… Canonical "published" marker key (what debug/spec expects)
  try {
    const key = publishedKey(projectId);
    const payload = {
      ok: true,
      projectId,
      publishedAtIso: new Date().toISOString(),
      source: "pages/api/projects/[projectId]/agents/auto-publish.ts",
      gitSha,
      buildTag,
    };
    ...
    // do not fail publish if marker write fails; return it as step info
    steps.push({
      name: "write-published-key",
      ok: false,
      ms: 0,
      error: `Failed to write published key: ${String(e?.message || e)}`,
    });
  }
